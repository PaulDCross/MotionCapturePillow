...
while True:
    ...
    if ret:
        ...
        if record:
            ...
            fps               = 10
            DIRV = os.path.join(directory, 'VideoOfVectorField_{0}FPS'.format(fps))
            makedir(DIRV)
            DIRR = os.path.join(directory, 'RawImage_{0}FPS'.format(fps))
            makedir(DIRR)
            DIRF = os.path.join(directory, 'Frame_{0}FPS'.format(fps))
            makedir(DIRF)
            numberofVideosV    = len([name for name in os.listdir(DIRV) if os.path.isfile(os.path.join(DIRV, name))])
            numberofVideosR    = len([name for name in os.listdir(DIRR) if os.path.isfile(os.path.join(DIRR, name))])
            numberofVideosF    = len([name for name in os.listdir(DIRF) if os.path.isfile(os.path.join(DIRF, name))])
            fourcc            = cv2.VideoWriter_fourcc(*'DIVX')
            VideoOfVectors    = cv2.VideoWriter(os.path.join(DIRV, str(numberofVideosV) + '.avi'), fourcc, fps, (933,654))
            VideoOfRawImage   = cv2.VideoWriter(os.path.join(DIRR, str(numberofVideosR) + '.avi'), fourcc, fps, (1184,656))
            VideoOfFrame      = cv2.VideoWriter(os.path.join(DIRF, str(numberofVideosF) + '.avi'), fourcc, fps, (933,502))
            record, recording = 0, 1
            print fps, numberofVideosV, numberofVideosR, numberofVideosF
            print "Recording"
        if initialize:
            print "Initialising"
        ...
        try:
            ...
            rec                 = Pillow(copy.deepcopy(image), refPt)
            ...
            Frame               = copy.deepcopy(image[y1:y2, x1:x2])
            ...
            Frame2              = cv2.drawKeypoints(Frame, keypoints, np.array([]), (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
            ...
            for data in DATA:
                ...
                # Draw on the Image
                cv2.putText(Frame2, "%d" % data[0], (int(data[4]) - 7, int(data[5]) - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1, 8)
                if data[7]:
                    # Draw the Line
                    cv2.line(Frame2, (int(data[1]), int(data[2])), (int(data[4]), int(data[5])), (0, 0, 255), 2)
                    cv2.circle(Frame2, (int(data[4]), int(data[5])), 1, (0, 0, 255), 2)


            frame_with_box[y1:y2, x1:x2] = Frame2
            ...
            # Help Text
            fontsize = 0.5
            Space = cv2.getTextSize("Press 'Space' to reset.", cv2.FONT_HERSHEY_SIMPLEX, fontsize, 1)
            cv2.putText(VectorField, "Press 'Space' to reset.", (5, VectorField.shape[0] - blackBar.shape[0] + Separation), cv2.FONT_HERSHEY_SIMPLEX, fontsize, (255,255,255))

            TAB = cv2.getTextSize("Press 'TAB' to increase vector lines.", cv2.FONT_HERSHEY_SIMPLEX, fontsize, 1)
            cv2.putText(VectorField, "Press 'TAB' to increase vector lines.", (5, VectorField.shape[0] - blackBar.shape[0] + Separation + 10 + TAB[0][1]), cv2.FONT_HERSHEY_SIMPLEX, fontsize, (255,255,255))

            if recording:
                rec = cv2.getTextSize("Press 'r' to stop recording.", cv2.FONT_HERSHEY_SIMPLEX, fontsize, 1)
                cv2.putText(VectorField, "Press 'r' to stop recording.", (5, VectorField.shape[0] - blackBar.shape[0] + Separation + 20 + TAB[0][1] + rec[0][1]), cv2.FONT_HERSHEY_SIMPLEX, fontsize, (255,255,255))
            else:
                rec = cv2.getTextSize("Press 'r' to start recording.", cv2.FONT_HERSHEY_SIMPLEX, fontsize, 1)
                cv2.putText(VectorField, "Press 'r' to start recording.", (5, VectorField.shape[0] - blackBar.shape[0] + Separation + 20 + TAB[0][1] + rec[0][1]), cv2.FONT_HERSHEY_SIMPLEX, fontsize, (255,255,255))

            saving = cv2.getTextSize("Press 's' to save pictures.", cv2.FONT_HERSHEY_SIMPLEX, fontsize, 1)
            cv2.putText(VectorField, "Press 's' to save pictures.", (5, VectorField.shape[0] - blackBar.shape[0] + Separation + 30 + TAB[0][1] + rec[0][1] + saving[0][1]), cv2.FONT_HERSHEY_SIMPLEX, fontsize, (255,255,255))

            esc = cv2.getTextSize("Press 'esc' to exit.", cv2.FONT_HERSHEY_SIMPLEX, fontsize, 1)
            cv2.putText(VectorField, "Press 'esc' to exit.", (5, VectorField.shape[0] - blackBar.shape[0] + Separation + 40 + TAB[0][1] + rec[0][1] + saving[0][1] + esc[0][1]), cv2.FONT_HERSHEY_SIMPLEX, fontsize, (255,255,255))
            ...
            if recording:
                ...
                VideoOfRawImage.write(image)
                VideoOfFrame.write(Frame)
        except IndexError:
            initialize = 1
            time.sleep(4)
    ...
    if k == 114: # r
        if recording:
            ...
            VideoOfRawImage.release()
            VideoOfFrame.release()
        else:
            record = 1
    if k == 115: # s
